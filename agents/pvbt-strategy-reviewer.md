...
- **Peak-of-grid defaults.** Default values that look grid-searched rather than chosen from theory: oddly precise decimals (`0.0173`, `-1.25`, `0.94`), magic integers without round-number grounding (`19`, `42`, `7` where `5` or `10` would have been natural), or a cluster of independent defaults that all sit suspiciously close to the best cell of a plausible sweep. A strategy whose parameter defaults cannot be explained by the hypothesis, the original paper, or a round-number convention is probably holding the peak of a grid search rather than the center of a robust region. Cite `../references/common-pitfalls.md#optimization-disguised-as-robustness-testing`.
- **Filter/gate accretion.** Parameters whose names (`trend-filter`, `regime-gate`, `vol-cap`, `dd-stop`, `entry-threshold`) describe a suppression mechanism rather than the core signal, especially when the strategy's one-sentence hypothesis does not mention them. Each such parameter is likely a patch for a specific losing period in backtest and is unlikely to generalize. Cite `../references/common-pitfalls.md#optimization-disguised-as-robustness-testing`.
